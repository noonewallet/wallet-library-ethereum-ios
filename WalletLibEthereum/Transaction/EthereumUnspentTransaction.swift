...
public class EthereumUnspentTransaction {
    ...
    public init(chainId: UInt64, nonce: Int, amount: Data, address: String, gasPrice: Data, gasLimit: Data, data: Data) {
    ...
    /// - Parameter data: Additional data
    convenience public init(chainId: UInt64, nonce: Int, amount: String.DecimalString, address: String, gasPrice: String.DecimalString, gasLimit: String.DecimalString, data: String.HexString) {
    
        self.init(chainId: chainId,
                  nonce: nonce,
                  amount: Data(hex: HexConverter.convertToHexString(fromDecimalString: amount)).dropedPrefixZeros,
                  address: address,
                  gasPrice: Data(hex: HexConverter.convertToHexString(fromDecimalString: gasPrice)),
                  gasLimit: Data(hex: HexConverter.convertToHexString(fromDecimalString: gasLimit)),
                  data: Data(hex: data))
